                    ...
                    <article>
                        <div class="kratos-hentry kratos-post-inner clearfix">
                            ...
                            <footer class="kratos-entry-footer clearfix">
								<?php if ( kratos_option( 'qq_group_prom' ) == 1 ) : ?>
                                    <span>QQ群开通：<?php echo kratos_option( 'qq_group_number' ); ?> <a target="_blank"
                                                                                                     rel="nofollow"
                                                                                                     href="<?php echo kratos_option( 'qq_group_code' ); ?>"><img
                                                    border="0"
                                                    src="<?php echo get_template_directory_uri() . '/images/qq-group.png' ?>"
                                                    alt="qq-group"></a></span>
								<?php endif; ?>
								<?php if ( kratos_option( 'post_statement' ) == 1 ) : ?>
                                    <div class="entrymeta">
										<?php $custom_fields = get_post_custom_keys( $post_id );
										if ( ! in_array( 'copyright', $custom_fields ) ) : ?>
                                            <h3>原创声明</h3>
                                            <span class="spostinfo">
该日志由 <a href="<?php echo get_option( 'home' ) ?>" title="<?php bloginfo( 'name' ); ?>"><?php bloginfo( 'name' ); ?></a> 于<?php the_time( 'Y年m月d日' ) ?>
                                                发表在<?php the_category( ', ' ) ?>分类下，
												<?php if ( ( 'open' == $post->comment_status ) && ( 'open' == $post->ping_status ) ) { ?>
                                                    你可以<a href="#respond">发表评论</a>，并在保留<a
...
<span class="content_tag"><?php the_tags( '关键字: ', ', ', '' ); ?></span>
                                            ...
                                            <div class="clear"></div>
										<?php else: $custom = get_post_custom( $post_id );
...
该日志转载于<a target="_blank" rel="nofollow" href="<?php echo $custom_value[0] ?>"><?php echo $custom_value[0] ?></a>由<a
                                                        ...
                                                        title="<?php bloginfo( 'name' ); ?>"><?php bloginfo( 'name' ); ?></a>整理编辑<br/>
...
<span class="content_tag"><?php the_tags( '关键字: ', ', ', '' ); ?></span>
								...
								<?php if ( kratos_option( 'wechat_prom' ) == 1 ) : ?>
                                    <div class="weixin-footer">
                                        <div class="weixin-content">
                                            <img src="<?php echo kratos_option( 'krsort_hm_img' ); ?>"
                                                 alt="weinxin"/>
                                            <div class="weixin-h"><strong>我的微信公众号</strong></div>
                                            <div class="weixin-h-w"><?php echo kratos_option( 'wechat_desc' ); ?></div>
                                            <div class="clear"></div>
                                        </div>
                                    </div>
								<?php endif; ?>
						...
						<?php if ( kratos_option( 'post_cc' ) == 1 ) : ?>
                            <div class="kratos-hentry kratos-copyright text-center clearfix">
                                <img alt="知识共享许可协议"
                                     src="<?php echo get_template_directory_uri(); ?>/images/licenses.png">
                                <h5>本作品采用 <a rel="license nofollow" target="_blank"
                                             href="http://creativecommons.org/licenses/by-sa/4.0/">知识共享署名-相同方式共享 4.0
                                        国际许可协议</a> 进行许可
                                </h5>
                            </div>
						<?php endif; ?>
						...
						<?php if ( kratos_option( 'post_related' ) == 1 ) : ?>
									...
									if ( $posttags ) {
										...
										while ( have_posts() ) {
                                            ...
                                            <li><a rel="bookmark" href="<?php the_permalink(); ?>"
                                                   title="<?php the_title(); ?>"
                                                   target="_blank" style="color: #00ccff;"><?php the_title(); ?></a>
                                            </li>
											<?php
											$exclude_id .= ',' . $post->ID;
											$i ++;
						...
						<?php endif; ?>
						<?php if ( kratos_option( 'post_bottom_ad' ) == 1 ) :
							echo kratos_option( 'post_bottom_ad_code' ) ?>
						<?php endif; ?>
